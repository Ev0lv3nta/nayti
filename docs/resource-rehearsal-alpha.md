...
Обновление 8 сентября 2026: этот документ сохраняет исторические измерения. Они **не пересняты на конечном polish commit**. Новые startup-сценарии находятся в [benchmark](../benchmark/README.md), E2E corpus и runner — в [evaluation](../evaluation/README.md); оба device-прогона пока NOT RUN. Не переносить приведённые ниже числа в метрики новой alpha.2.
